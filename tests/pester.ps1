...
describe 'Testrun with alternate logfile location' {
        it 'should succeed' {
		$error.Clear()
		iex '.\file-archiver\archive.ps1 -configFile .\file-archiver\tests\fixtures\archive-config.xml -logfile .\tests\logfile.log -configFile .\tests\fixtures\archive-config.xml'
		$error | Should -Be $Null
        }
